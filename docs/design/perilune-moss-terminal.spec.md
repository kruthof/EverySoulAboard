...
| **IX-M15** | **⭐ Added 2026-08-04 by `moss-autoscroll`.** The `>` transcript **follows its newest line**, because OD-P made this a real terminal and a terminal that hides its own output is broken. The rule is the standard **pinned-to-bottom** idiom, and it has two halves: on new content the pane scrolls to the bottom **iff the view was at (or within a slack of) the bottom before the append**; a player who has deliberately scrolled up to read history **keeps their position** until they return to the bottom, at which point the follow re-arms. ⛔ A naive always-jump is explicitly refused: renders arrive on every pushed `systems`/`log`/`chron` message and on the POD BAY's 1 Hz poll, so "always scroll to the bottom" would drag a reader off their place about once a second with no output to justify it. The decision is the pure `shouldFollowTail(scrollTop, clientHeight, scrollHeight)` in `moss-screen.js`, asked **before** the rebuild — after it, `scrollHeight` already counts the appended lines, so "was the player at the bottom?" would answer *no* whenever anything arrived and the console would never follow again. ⚠️ **The FOLLOW arm is the whole of the fix**; the no-move arm is defence-in-depth, kept deliberately (integrator ruling 2026-08-04) and pinned at a non-zero scroll position. ⛔ **Do not repeat the retracted claim that `replaceChildren` clamps `scrollTop` to 0** — it was in this row's first draft and it is FALSE, measured in Chrome on the shipped pane: parked at 357 of a 714 maximum it reads 357 after the rebuild, still 357 when a layout read is forced while the box is empty, and 357 across six real 1 Hz rebuilds. The pane sat at 0 only because nothing had ever scrolled it and new lines append below the fold. *Defect it closes, measured 1280×800 on the shipped wreck: `help` printed 14 lines into the `max-height:22vh` pane — `clientHeight 157 / scrollHeight 305 / scrollTop 0` — and the hidden half was the bottom half, i.e. COMMISSION, PODS and THAW.* |
| **IX-M16** | **⭐ Added 2026-08-04 by `moss-scroll-affordance`.** When lines sit below the fold of the `>` transcript, the pane **says so**: a passive **“▾ N MORE”** sign over its bottom edge, with a fade to the page colour so the clipped line reads as CONTINUING. It refreshes on every scroll and after every render, and it is **hidden the moment nothing is below the fold** — including at the tail the follow of IX-M15 just reached. ⛔ **It is a SIGN, NOT A CONTROL (OD-P).** Every printable character belongs to the prompt, so the affordance adds no navigation the terminal does not already have: `pointer-events:none`, `aria-hidden`, no listener, not focusable; the wheel and `PageUp`/`PageDown`/`Space` keep exactly the semantics they had. **N is counted from EACH LINE'S OWN BOX, never from a row stride** — the CREW-tab affordance this copies (`hud.js:updateCrewMore` + `console-model.moreBelow`) may divide by one stride because every crew row is the same height, and MOSS lines are not: measured in Chrome on the shipped wreck at 1280×800, a HELP column is 21.77px and the offline gate sentence is **43.53px, exactly two line boxes**. So the pure decision is a SIBLING, `linesBelowFold(rowBottoms, foldTop, foldBottom)`, and it degrades the OPPOSITE way to `shouldFollowTail`: an unmeasurable pane prints nothing, because a wrong number is worse than silence. ⛔ **AND THE CLEARANCE MUST BE INVISIBLE TO IX-M15.** The sign reserves room for the last line with `padding-bottom` on the SCROLLER, and a browser counts padding in `scrollHeight` — which is the unit `shouldFollowTail` is denominated in. Shipped naively (4edb183) that redefined `TAIL_SLACK_PX` behind IX-M15's back: measured 23.52px of clearance against a 24px slack leaves an effective ~1px, so **the follow-within-slack arm was unreachable whenever the sign was up** — driven A/B, parked at d = 10.48 with `▾ 1 MORE` showing, the build HELD where main FOLLOWED. The rule is therefore: *whatever a later lane does for clearance, subtract it before asking the follow question* — the follow is about CONTENT, and padding is not content. Pinned by a node test in the band `1px < d ≤ 24px` (which nothing covered before) and by `moss-scroll-shot.mjs` STEP 6, which drives both A/B cells and requires them to agree. *Gap it closes: IX-M15 made the console follow its newest line, but a parked reader — or one on `HELP`, 14 lines in a ~7-line pane (13 when this row was written; the `vents` lane added one) — still had no signal that anything was below the fold, and `overflow-y:auto` shows no resting scrollbar here.* |
