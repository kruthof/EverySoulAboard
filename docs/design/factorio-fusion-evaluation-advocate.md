...
| **Ratio balancing** | 2 furnaces feed 3 assemblers; get the numbers right and nothing starves or backs up | **Yes.** Loss is an integer input:output ratio by design (`ProductionDefs.cs:6-18` — `Scrap:20→Regolith:17` is exactly 85%). Balancing converter throughputs so nothing strands is *literally* the failure `ECONOMY.md` §8 measured (wrong-deck stockpile → −14% throughput). |
