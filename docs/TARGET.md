...
| T8 | A stalled/refused order says WHY, on the tile | §2.4 (`JobCondition`) | partial, advanced by M3-13 (08-01) and by D5 (08-03): the `blocked` badge NAMES THE ITEM a stalled repair waits for (`NEEDS PARTS — …`, `BlockedCell.Detail`), the right-click menu refuses never-serviceable machines out loud, and ⭐ an accepted order the sim then DROPS no longer evaporates — an ordered machine the crew cannot walk to wears `ReasonNoRoute` / *NO WAY TO WALK TO IT* from the frame after the click and keeps it after the job dies (§13.25 b3). Remaining silent: the same arm when the route closes MID-ORDER (the record is retired by then — §13.25 b3's residual), the APPROACH refusal (no staging tile at all), the other §13.25 b2 refusals, and the other EIGHT `DriveWorker` abandon arms as a class |
