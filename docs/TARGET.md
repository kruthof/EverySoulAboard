...
| T5 | Right-click "Prioritise: repair X" forced order | §2.2 | DONE (M2-9 + M2-10; demo step 6: order crossed the ship and landed). M3-14 (07-31): a held order now crosses the pressure frontier — RW§8.4 rungs 2+3+4; she does not flee and may die, pinned by name. Names the device TYPE, not the authored instance — owner item |
...
| T8 | A stalled/refused order says WHY, on the tile | §2.4 (`JobCondition`) | partial, advanced by M3-13 (08-01): the `blocked` badge now NAMES THE ITEM a stalled repair waits for (`NEEDS PARTS — …`, `BlockedCell.Detail`), and the right-click menu refuses never-serviceable machines out loud instead of toast-then-silence; remaining silent: the APPROACH refusal for a repair order, other refusals per §13.25 b2 |
...
| T13 | Thaw loop: earn & choose a second soul through MOSS | premise | near-DONE (M3-2 + M3-3 + M3-4, merged 08-01): typed `pods` on the commissioned console shows the bay, every sealed row states the gate's reason and number, and a thaw chosen there CYCLES — witnessed in real Chrome on the shipping ship (commissioning driven via a disclosed temp overlay; the unmodified commission chain is playable but long). Remaining before DONE: M3-13 (tile-badge refusals + never-serviceable menu), M3-5, and one witnessed unmodified-game run |
