...
| T12 | Needs/mood: rest, food; mood as consequence not meter | §4 | ⭐ **DONE for the headline half, merged 2026-08-06 by M4-9** (MECHANICS §13.51): **mood gates BEHAVIOUR** — a deterministic three-tier ladder (RW§4.2's one-tunable derivation, ×4/7, ×1/7, clamped 1–50 %) over hashed mood with a DWELL instead of a rate and no roll anywhere; she refuses the dangerous order / stops working / withdraws, graduated player override (OD-S item 3 = A). ⭐ *"no mood freeze while asleep"* CLOSED too — the dwell counter pauses on `JobKind.Sleep`. Earlier: M3-9 (08-02) crew SLEEP; fatigue → mood → Director tension → machine wear (a consequence, no meter). **Remaining: eating loses to work** (`SustenanceSystem` after `JobSystem`, filed) — and note that this is the SAME gate the break arrives through: a held crew member does not eat, so a standing order starves her to the deprivation floor in ~21 sim-h and she breaks (driven, `MentalBreakTests`). ⚠️ **Also remaining: the shipping wreck left alone never reaches the first rung** (measured, 21 sim-days) — scarcity tuning is M5's by the M4 charter's own ruling |
...
| T14 | Health: capacity-gated work (downed ≠ disabled) | §6.1 | missing — but **the real-or-delete question is DECIDED: REAL** (OD-S item 2, 2026-08-05; `Citizen.Health`/`Morale`/`Archetype` all become real, RW§6.1's safety-net shape, and the ruling accepts the pin cost — there was no zero-pin option). **M4-4 now IMPLEMENTS rather than decides**; pin row `pin/m4-a` expects P1/P2/P3 and possibly P4/P5 |
...
| T16 | Persona window (ONE door to a person) | — | ⭐ **SHIPPED 2026-08-05 by M4-2** — one click, one window, key `U`; the three doors became one (`CREW_INTERACTION` = `['openPersonaForSelected']`, censused). Six bands, and **every line is named to its wire source or written as an honest empty** (`persona-view.js:48-64`): WHO SHE IS · DOING & WHY (the roster `task` VERBATIM, no ellipsis — this is the surface M2-6's clipped why-clause was filed to) · **HOW SHE IS** (M4-9's band, `GameSession.HowSheIs`) · CAN & CANNOT (RW§1.6's *Incapable Of*) · TIES & HISTORY. Design charter M4-1 written 08-04; §10 batch ANSWERED by OD-S. **KNOWN LIMITS, both named packages**: the written identity + prose bonds wait on **M4-3** (`PersonaSheet` is host-owned, on no wire) and her Chronicle lines on **M4-7** — the window draws honest empties, never a `◇ SAMPLE`, but `panels.js`' deprecated dossier still carries the SAMPLE ledger, so the M4 gate's *no ◇ SAMPLE anywhere* clause is NOT yet met |
