...
| T13 | Thaw loop: earn & choose a second soul through MOSS | premise | partial (M3-2 the pod cycles + M3-3 the gate, merged 08-01): ask via the MOSS `thaw` op and the ship answers yes or no-with-a-named-reason-and-number — six terms, price charged last, rungs consumed via `ThawGate` (§13.30). NOT yet reachable from the browser UI (no client sender) — the POD BAY screen is M3-4, refusal surfacing M3-13; the row moves DONE only when a thaw happens in `./play.sh` |
