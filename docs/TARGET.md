...
| T12 | Needs/mood: rest, food; mood as consequence not meter | §4 | advanced by M3-9 (08-02): crew SLEEP — she finishes her job, walks to a bunk (or the deck, worse), sleeps ~8-10 sim-h, wakes, works; fatigue reaches mood → Director tension → machine wear (a consequence, no meter). Remaining: mood still gates no crew behaviour directly — **OD-R (08-04) DIRECTS this half, and the M4 charter (same day, `perilune-m4.packages.md` TWoM-1) now DESIGNS it: a dwell/decaying-counter ladder over hashed mood, RW§4's one-tunable derivation, priced in fields; first break's builder = owner batch item 1, first required measurement = the post-M3-9 mood envelope + sawtooth amplitude/period (nobody has re-measured it)**; eating loses to work (SustenanceSystem after JobSystem, filed); no mood freeze while asleep |
