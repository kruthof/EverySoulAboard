...
| R | **THE THIRD REGISTER — THIS WAR OF MINE** (owner-direct, 2026-08-04, in conversation): the target gameplay is **RimWorld × Factorio × This War of Mine**. TWoM names the survival REGISTER — scarcity that cannot be fully relieved, who-to-wake / who-to-risk triage, consequences carried by named people and told by the Chronicle, endurance not power fantasy — **not a mechanism source: RimWorld stays the mechanism authority** (unchanged); TWoM is the tonal authority for how **M4 (THE PERSON)** and **M5 (the ending)** get charted, and every M4/M5 package charter names which register it serves. The premise already carries the register (everyone awake is dead · thaw triage · over-thaw punishes itself · `NO AIR — SHE MAY DIE`), so this row mostly AIMS existing queue rows, it does not add any. ⛔ **Scope clause, OD-P's shape: NOTHING is implementable from this row** — no moral-choice engine, no mental-break table, no scavenge cycle, no misery meter (TARGET §2 gains the anti-goal); file ideas against M4-1/M5-1, never implement from here. Recorded with the same-day drift check: PROCESS §2 gains the lane-selection gate + `rows:` disclosure | TARGET §1/§2 amended · PROCESS §1/§2 amended · M4-1 and M5-1 charters (written at M4 open, aimed by this row) |
