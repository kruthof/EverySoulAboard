...
| **M4 — THE PERSON** | 10–11 | One click → one Persona window: who she is, what she's doing, why, how she is — no `◇ SAMPLE` anywhere; Chronicle reachable. **PARTIAL 2026-08-06**: the window ships and answers all four questions (M4-2 + M4-9's HOW SHE IS band); the two remaining clauses are **M4-3** (`panels.js` still carries the SAMPLE ledger) and **M4-7** (the band draws an honest empty, not her lines). |
...
| ✅ | M2-6 | the `why` line — merged 2026-07-30 (after M2-8; deliberate swap, disjoint files). One send-back: the clause measured INVISIBLE in both crew docks (26/23-char ellipsized cells vs 43–54-char lines); fixed by the M2-20 precedent — docks render the WHAT half at the declared ` — ` separator (`GameSession.RankingSeparator` ↔ `WHY_SEPARATOR`), the full WHAT—WHY reads in the Overview selected readout. Room Zoom has NO readout at all → clause unreachable there, filed to M4 Persona. ✅ **THAT FILING IS CLOSED 2026-08-05 by M4-2**: the Persona window's DOING & WHY band renders the roster's `task` field VERBATIM — no prefix, no `watchTask` derivation, no ellipsis (`persona-view.js:167-180`, equality asserted against the wire field), and the D5 stuck-reason sits under it from the SAME `crewBlockedOrder` join | neutral DONE |
...
- **M4:** ✅ **CHARTERED IN FULL, 2026-08-04 — `docs/design/perilune-m4.packages.md`**
  ...
  ANSWERED — OD-S, 2026-08-05, all five verbatim; M4 IS OPEN.** ⛔ **The charter's clause
  "the 08-07 playtest's findings amend the charter before M4-2 implements" is SUPERSEDED by
  OD-S**: M4-2 and M4-9 both merged 2026-08-05/06, so **the playtest's findings amend M4's
  LATER packages — M4-3 onward — and never the two already on `main`.**).
  The outline that follows is kept as the index.
  M4-1 Persona design (INFRA — doc written, §10 batch ANSWERED by OD-S; (a)–(h) stand) ·
  **M4-2 build it ✅ MERGED 2026-08-05** (one door, key `U`; pin-neutral) · M4-3
  dossier stops lying · M4-4 Health/Morale real-or-delete (OD-S item 2 = REAL) · M4-5
  onboarding rewrite · M4-6 RUG/SHELF · M4-7 Chronicle reachable · M4-8 WP-9 console
  deletion.
  ...
  ✅ **MERGED 2026-08-06** (MECHANICS §13.51): the deterministic ladder, the HOW SHE IS band that
  closes M4-2's PARTIAL *how she is* clause, the Chronicle line, and the graduated override.
  ⛔ Pins P1/P2/P3 MOVED, P4/P5 HELD — `pin/m4-b` written, CLAUDE.md + `ci.sh` + MECHANICS re-pinned.
  ⚠️ **THE M4 EXIT GATE IS NOT YET MET.** Its four questions are answered — *who she is · what
  she's doing · why* (M4-2) and *how she is* (M4-9's band) — but **two clauses remain**:
  *no `◇ SAMPLE` anywhere* is **M4-3**'s (the dossier still fabricates), and *Chronicle
  reachable* is **M4-7**'s (`persona-view.js:215-217` draws the band and the honest empty
  sentence, not the lines). ⇒ gate = **PARTIAL, two named packages short**.
...
| S | **THE M4 §10 BATCH — ACCEPTED VERBATIM, AND M4 IS OPEN** (owner-direct, 2026-08-05, in session): all five recommendations taken as written. **1** the first mental break is BUILT NOW, as a ninth package **M4-9** (not deferred to M5) — created by this ruling, not by the charter, per §4's numbering rule. **2** `Citizen.Health`/`Morale`/`Archetype` go **REAL** (RW§6.1's safety-net shape), never keep-and-stop-showing; there was no zero-pin option and the ruling accepts the pin cost. **3** a player order **MAY** override a break, **graduated by tier** — not RimWorld's flat no. **4** thawed sleepers arrive with **real SOCL bonds, seeded in `CryoSystem`** (bonds stop being prose; D-3's Feud arm survives). **5** the Chronicle **severity tie is KEPT, earliest wins** (= the silence default). ⭐ **THE ECONOMY STAYS PARKED** — OD-B is untouched and the faucet ruling is still open on the owner; nothing in this batch re-opens E1. ⛔ **AND IT RE-SEQUENCES THE PLAN**: the charter's clause *"the 08-07 playtest's findings amend this charter BEFORE M4-2 implements"* is **OVERRIDDEN** — M4-2 and M4-9 both shipped 2026-08-05/06 under this ruling, so the playtest's findings amend **M4's LATER packages (M4-3 onward)**, never the two that are merged. Design questions (a)–(h) stand as the charter's calls | **M4-9** (new; merge-order position 4b, pin row `pin/m4-b`, runs alone — MOVED P1/P2/P3, MERGED 2026-08-06) · **M4-2** unblocked and MERGED · **M4-4** bound to item 2 (real) · **M4-3** bound to item 4 (seeded bonds) · closes T12's OD-R half and the M4 gate's *how she is* clause · amends the M4 charter's playtest clause · **does NOT touch OD-B** |
