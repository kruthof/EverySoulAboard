...
⚠️ **PORTED ONTO `rig-lib.mjs` AT MERGE TIME, and that is a real fix rather than tidying.** The tool
was written against the one-shot onboarding dismissal; `main`'s tooling lane landed
`client/tools/rig-lib.mjs`'s `dismissOnboarding` (verified-gone, laid-out-rect) while this lane was
open. Left alone, this would have shipped as a NEW rig carrying the exact defect that lane closed —
a one-shot that silently skips a card which has not painted, after which every click lands on a
full-screen modal instead of the ship. The witness run below was re-run on the merged tree through
the ported helper.
