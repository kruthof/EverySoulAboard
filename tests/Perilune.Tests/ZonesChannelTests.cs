...
namespace Perilune.Tests
{
    ...
    public class ZonesChannelTests
    {
        ...
        /// <see cref="ItemKind"/>s, so today's ceiling is 0x17F and the channel is safe.
        ///
        ...
        public void AcceptMask_StaysInsideTheJsonSafeIntegerRange()
        {
            ...
            // 0x17F = bits 0-6 (Regolith..ControllerModule) plus bit 8 (Ice). Bit 7 is CLEAR: the
            // integrator reserved that slot for E0-6's Seals and E0-7 deliberately left the hole
            // rather than renumbering a byte enum that is written into every save (E0-7).
            Assert.AreEqual(0x17FUL, StockZoneSystem.AcceptAllMask,
                "eight ItemKinds today, with a hole at 7; if this changed on purpose, re-measure the " +
                "bound above with it");
        }
