...
namespace Perilune.Tests
{
    ...
    public class IceMelterTests
    {
        ...
        /// <summary>
        /// <see cref="ItemKind.Ice"/> is 8, NOT 7. The integrator pre-assigned slot 7 to the sibling
        /// E0-6 lane's <c>Seals</c> so the two lanes could not collide on a byte that is written into
        /// every save (ECONOMY-PLAN §2.1 rule 2). Pinned by NAME here because "the enum has a hole"
        /// is a fact several masks depend on, and because a well-meaning tidy-up that closes the gap
        /// would silently re-label every stack in every existing save.
        ///
        /// MUTATION THAT MAKES THIS FAIL: renumber <c>Ice = 7</c> — this test names the reason.
        /// </summary>
        [Test]
        public void IceIsKindEight_LeavingSevenReservedForTheSiblingLane()
        {
            Assert.That((int)ItemKind.Ice, Is.EqualTo(8));
            Assert.That(Enum.IsDefined(typeof(ItemKind), (byte)7), Is.False,
                "slot 7 is RESERVED for E0-6's Seals and must stay empty until that lane merges");
