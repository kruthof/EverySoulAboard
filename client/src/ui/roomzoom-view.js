 ...
 *
 * THIS ACCESSOR IS KEPT ANYWAY, and not because it is used internally — it is not. It is the tested
 * seam other code (a tooltip, a future inspector) reads a tile's wear through without importing the
 * channel decode, and `client/test/surface-boundary.test.js` names it. Read from `_deviceCond`,
 * which `repaint()` refreshes once per frame from the live channel.
 */
