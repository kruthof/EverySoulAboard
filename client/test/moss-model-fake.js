 ...
 * be a second one. (During the parallel build this file carried a stand-in copy of rowObj's rules,
 * because `moss-model.js` was still a frozen stub; that fallback was deleted at merge, once the real
 * `rowObj` landed. A missing state is `-1`/UNKNOWN, NEVER `0`/NOMINAL — DA-M1 forbids inventing a
 * healthy reading for a row we cannot read.)
 */
const rowObj = MODEL.rowObj;
